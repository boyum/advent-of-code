...
const totalSize = getAllFileSizesFromDirectory(rootDirectory).reduce(
  (acc, size) => acc + size,
  0,
);

const neededSpace = 30_000_000 - (70_000_000 - totalSize);
const smallestPossibleDir = Math.min(
  ...dirSizes.filter(size => size >= neededSpace),
);
console.log({ smallestPossibleDir });
