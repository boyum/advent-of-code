...
const aContainsB = <T extends string | boolean | number>(
  a: Array<T>,
  b: Array<T>,
): boolean => {
...
const pairsWhereOneIsContainedByTheOther = pairs.reduce((accumulator, pair) => {
  ...
  const oneContainsTheOther =
    aContainsB(aSections, bSections) || aContainsB(bSections, aSections);

  return accumulator + (oneContainsTheOther ? 1 : 0);
}, 0);
